...
------------------------------------------------------------------------------
--- How to add functions to hscLegacy.lua 
--- Use hscLegacy.isPlayerInsideVolume and hscLegacy.aiPlace as examples
------------------------------------------------------------------------------
local hscLegacy = {
    ai = {},
    unit = {}
...
function hscLegacy.isPlayerInsideVolume(volumeTriggerName)
...
function hscLegacy.bspIndex(global)
    execute_script("set " .. global .. " (structure_bsp_index)")
    return get_global(global)
end

function hscLegacy.gamesafe()
  local gameSafe = [[
...
---@return number
function hscLegacy.aiLivingCount(encounterName, globalVar) 
...
function hscLegacy.aiSpawn(type, encounterName)
...
function hscLegacy.aiMigrate(from, to)
...
function hscLegacy.aiMigrateByUnit(unit, ai)
    execute_script("ai_migrate_by_unit " .. unit .. " " .. ai)
end

function hscLegacy.aiCommandList(ai, list)
...
function hscLegacy.aiMagicallySee(script, sourceEncounter, targetObj)
...
function hscLegacy.aiMagicallySeePlayers(ai)
...
function hscLegacy.AllegianceSet(team1, team2)
    if (team1) and (team2) then
        ...
        console_out("Parameter missing from hscLegacy.allegianceSet function")
    end
end

--- Remove AI Allegiances
function hscLegacy.AllegianceRemove(team1, team2)
    if (team1) and (team2) then
        assert((type(team1) == "string" or type(team1) == "number") and (type(team2) == "string" or type(team2) == "number") , "Please use a string or a number to declare the teams")
        local t1 = team1
        local t2 = team2
...
function hscLegacy.AllegiancesGet(team)
...
--- AI Behaviour
---
function hscLegacy.aiAction(type, encounterName)
    local returnType = {"follow_target_players", "attack", "defend"}
    execute_script("ai_" .. returnType[type] .. " " .. encounterName)
end

function hscLegacy.aiState(type, encounterName, boolean)
...
function hscLegacy.aiDisregard(objectList, boolean)
    execute_script("ai_disregard " .. objectList .. " " .. boolean)
end

function hscLegacy.aiExitVehicle(ai)
    execute_script("ai_exit_vehicle " .. ai)
end

function hscLegacy.aiVehicleEntrableDistance(vehicle, real)
    execute_script("ai_vehicle_enterable_distance " .. vehicle .. " " .. real)
end

function hscLegacy.aiVehicleEncounter(vehicle, encounter)
...
function hscLegacy.aiAttach(biped, ai)
...
function hscLegacy.unitEnterVehicle(unit, vehicle, marker)
    execute_script("unit_enter_vehicle " .. unit .. " " .. vehicle .. " " .. marker)
end

--- Unit Exit vehicle
---@param1 Unit to eject
function hscLegacy.unitExitVehicle(unit)
    execute_script("unit_exit_vehicle " .. unit)
end

--- Vehicle load magic
function hscLegacy.vehicleLoadMagic(vehicle, seat, ai)
...
function hscLegacy.vehicleLoadNoSeat(vehicle, ai)
    execute_script("vehicle_load_magic " .. vehicle .. " " .. ai)
end

--- Vehicle unload magic
function hscLegacy.vehicleUnload(vehicle, seat)
...
function hscLegacy.customAnimation(vehicle, model_animation_path, animation_name, boolean)
...
function hscLegacy.recordingAnimationHover(vehicle, recorded_animation_name)
...
function hscLegacy.objectsAttach(parent, pMarker, child, cMarker)
...
---@param1 Parent Object
---@param2 Child Object
function hscLegacy.objectsDetach(parent, child)
...
function hscLegacy.objectTeleport(objectName, cutsceneFlag)
...
function hscLegacy.actionTest(type)
...
---@param1 boolean
function hscLegacy.playerAddEquipment(unit, starting_profile, boolean)
...
function hscLegacy.soundImpulseStart(source, object, gain)
...
function hscLegacy.soundImpulseStop(source)
    execute_script("sound_impulse_stop " .. source)
end

---@param Source - Sound file to be referenced
function hscLegacy.soundImpulseTime(source, global)
...
---@param2 object you want to play the sound from
---@param3 gain between 0 and 1 of how loud the sound is
function hscLegacy.soundLoopingStart(source, object, gain) 
    execute_script("sound_looping_start " .. source .. " " .. object .. " " .. gain)
end

--- Sound Looping stop
---@field source Source of sound file
function hscLegacy.soundLoopingStop(source)
...
---@param1 source
---@param2 boolean
function hscLegacy.soundLoopingAlternate(source, boolean)
...
function hscLegacy.screenEffectConvolution(strength, sharpness, initial, final, transition)
...
function hscLegacy.screenEffectVideo(intensity, border)
...
--- Cinematic letterbox
---@param1 boolean
function hscLegacy.cinematicLetterbox(boolean)
    execute_script("cinematic_show_letterbox " .. boolean)
end

--- Cinematic letterbox with no hud
--- @param1 boolean
function hscLegacy.cinematicLetterbox_noHud(boolean)
...
function hscLegacy.damageEffect(tag, unit, marker)
...
function hscLegacy.Fade(type, r, g, b, ticks)
...
--- Show hud
---@param1 boolean
function hscLegacy.showHud(boolean)
...
function hscLegacy.hudShowComponent(type, boolean)
...
---@param2 Boolean True/False
function hscLegacy.hudBlinkComponent(type, boolean)
...
---@param2 Minutes
---@param3 Seconds 
function hscLegacy.hudTimer(type, minutes, seconds)
...
function hscLegacy.activateNav(type, unit, source, verticalOffset)
...
function hscLegacy.clearNav(type, unit, source)
    local returnType = {"flag", "object"}
...
function hscLegacy.navpointEnemy(unit, ai, object)
...
---@param script Name
function hscLegacy.script(name)
...
--- @param3 Sleep
function hscLegacy.setCamera(camera_point, real, sleep)
...
--- Camera control
---@param1 boolean
function hscLegacy.cameraControl(boolean)
    execute_script("camera_control " .. boolean)
end

--- Makes the scripted camera zoom out around a unit as if it were dead.
---@param1 unit
function hscLegacy.cameraSetDead(unit)
    execute_script("camera_set_dead " .. unit)
end
------------------------------------------------------------------------------
--- Device Functions
------------------------------------------------------------------------------
-- Device functions have been simplified into "properties". Instead of having to write "device_get_power" you can simply write hscLegacy.deviceGet("Power" , "deviceName") or hscLegacy.deviceGet("position", "deviceName")
...
---@param2 Device Name
function hscLegacy.deviceGet(type, deviceName)
...
---@param2 Device Name
---@param3 Boolean
function hscLegacy.deviceSet(type, device, boolean)
    local returnType = {"power", "position"}
...
--- @param1 device name
--- @param2 boolean
function hscLegacy.deviceSetPosImmediate(device, boolean)
...
function hscLegacy.groundedOpen()
    local flash = [[(begin
    (show_hud 0)
...
------------------------------------------------------------------------------
-- The goal of this refactor is to move the functions into a method. It will start with hscLegacy.<object>:function(<param>) I will not be removing working code, so don't you remove it either. Or i'll hunt you down and stare at you, because I'm doing a hack version of refactoring. 
...
function hscLegacy.ai:allegiance(team1, team2, state)
    if (team1) and (team2) then
        assert((type(team1) == "string" or type(team1) == "number") and (type(team2) == "string" or type(team2) == "number") , "Please use a string or a number to declare the teams")
        local t1 = team1
        local t2 = team2
...
--- Alter various states of AI behaviour. Can set the AI to be: (Case sensitive)
--- berserk, attack, defend, braindead, playfight, blind, deaf, respawn (enables respawning defined in the encounter in the scenario)
...
---@param boolean? boolean optional boolean
function hscLegacy.ai:behaviour(behaviour, targetEncounter, boolean)
    ...
    if boolean then
        assert(type(boolean) == "boolean", "boolean" .. errorMessages.boolean)
    end
    local tempBool = ""
    local behaviourTable = {"berserk", "attack", "defend", "braindead", "playfight", "blind", "deaf", "respawn"}
    local booleanBehaviours = {behaviourTable[1], behaviourTable[4], behaviourTable[5], behaviourTable[6], behaviourTable[7], behaviourTable[8]}
    local setBehaviour = {behaviourTable[6], behaviourTable[7], behaviourTable[8]}
    ...
    if type(behaviour) == "number" then
        ...
        --local safe = false
        for i = 1, #behaviourTable do
            local str = string.find(behaviourTable[i], behaviour)
            if str then
                aiBehaviour = behaviourTable[i]
                break
            end
            if i == #behaviourTable then
                assert(false, "Incorrect behaviour input to paramter \"Behaviour\"")
            end
        end
    end
    for i = 1, #booleanBehaviours do
        if aiBehaviour == booleanBehaviours[i] then
    ...
    for i = 1, #setBehaviour do
        if aiBehaviour == setBehaviour[i] then
            aiBehaviour = "set_" .. aiBehaviour
        end
    end
    execute_script("ai_" .. aiBehaviour .. " " .. targetEncounter .. " " .. tempBool)
    console_out("ai_" .. aiBehaviour .. " " .. targetEncounter .. " " .. tempBool)
end
    ...
    Follow the player
        hscLegacy.ai:follow("player", "encounter1", nil, true)
    
    Disable following
        hscLegacy.ai:follow(nil, "encounter1", nil, false)

    Follow a unit at a specific distance
        hscLegacy.ai:follow("unit", "encounter1", "warthog1", true, 10)
    ]]
...
function hscLegacy.ai:follow(followType, sourceEncounter, target, state, distance) 
    ...
    if newState then
        if type(followType) == "number" then
            ...
            for i = 1, #followTable do
                local str1 = string.find(followTable[i], followType)
                if str1 then
...
--- Get the living count of AI in a specific encounter. Must use a global short that you declared in <yourmap>.hscLegacy. Globals MUST be lower case.
--- @param encounter string name of the encounter
---@param globalVar string Global variable declared in yourmap.hsc
function hscLegacy.ai:livingCount(encounter, globalVar) 
...
function hscLegacy.unit:getHealth(unit, globalVar)
...
function hscLegacy.unit:animate(unit, animation_graph, animation_string, boolean, startFrame)
